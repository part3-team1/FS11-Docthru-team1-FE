    ...
    <div className={styles.container}>
        ...
        <div ref={filterRef} className={filterStyles.dropdownContainer}>
          ...
          {isFilterOpen && (
            ...
            >
                ...
                <div>
                  ...
                  <div className={filterStyles.labelContainer}>
                    {FILTER_STATUS.map((item) => (
                      <label
                        key={item.value}
                        className={filterStyles.labelItem}
                      >
                        <input
                          type="radio"
                          name="filterStatus"
                        ...
                        <Image
                          src={
                            tempStatus === item.value
                              ? '/Images/Icon/radio_checked.svg'
                              : '/Images/Icon/radio_normal.svg'
                          }
                          alt=""
                          width={24}
                          height={24}
                        ...
                        />
                        <span className={filterStyles.labelText}>
                          {item.label}
                        </span>
                      </label>
                    ))}
              ...
              {/* 마감 기한 */}
              <div
                className={`${filterStyles.filterBoxWrapper} ${filterStyles.lastBox}`}
              >
                <div>
      ...
      <table className={styles.table}>
        ...
        <thead className={styles.tableHead}>
          <tr>
            <th
              className={`${styles.tableHeadCell} ${styles.tableHeadCellFirst}`}
            >
              No.
            </th>
          ...
          {requests.map((item, index) => {
            ...
            const handleClick = () => {
              ...
              sessionStorage.setItem(
                'challengeRequestNoToId',
                JSON.stringify(noToId),
              );
            };
            return (
              <tr key={item.id}>
                ...
                <td className={styles.tableBodyCell}>
                  {DOCUMENT_TYPE_LABEL[item.documentType] ?? item.documentType}
                </td>
                <td className={styles.tableBodyCell}>{item.category}</td>
                <td
                  className={`${styles.tableBodyCell} ${styles.tableBodyCellTitle}`}
                >
                  <Link
                    href={`/admin/challenge-management/${item.id}?no=${itemNo}&totalCount=${totalCount}`}
                    onClick={handleClick}
                  >
                    {item.title}
                  </Link>
